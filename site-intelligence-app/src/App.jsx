...
import { Search, BarChart3, Layout, Plus, X, ChevronDown, ChevronRight, GripVertical,
  ...
  ArrowRight, Layers, Target, FileText, Home, Bookmark, CheckCircle, Trash2 } from "lucide-react";
...
function useConfetti(){
  ...
  const fire=useCallback((x,y)=>{
    const cols=["#f36458","#37cd84","#55beff","#C9A96E","#ffffff","#f0f0f0","#ffdd44"];
    ...
    const tick=()=>{
      setParticles(p=>{
        const next=p.map(pt=>({...pt,x:pt.x+pt.vx,y:pt.y+pt.vy,vy:pt.vy+0.2,rot:pt.rot+pt.rotV,life:pt.life-0.02}))
          .filter(pt=>pt.life>0);
    ...
    <div onClick={e=>e.stopPropagation()} style={{background:T.canvasSoft,
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
        <div>
          ...
          <h3 style={{fontFamily:FONT_SANS,fontSize:16,fontWeight:400,color:T.onPrimary,margin:0,letterSpacing:"-0.2px"}}>Place on map</h3>
        </div>
      ...
      <div style={{marginBottom:20}}>
        ...
        <input value={label} onChange={e=>setLabel(e.target.value)}
          placeholder="e.g. Potential activation zone..." autoFocus={!addingCat}
          ...
          onKeyDown={e=>{if(e.key==="Enter"&&label.trim())onAdd({cat,label:label.trim(),mx:pos.mx,my:pos.my,id:"b"+Date.now()});}}/>
      </div>
      <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
        ...
        <button onClick={()=>{if(label.trim())onAdd({cat,label:label.trim(),mx:pos.mx,my:pos.my,id:"b"+Date.now()});}}
...
/* ═══════════════════════════════════════════════════════════════════
   SITE MAP — enhanced with check-off, bookmarks, richer detail
   ═══════════════════════════════════════════════════════════════════ */
function SiteMap({findings,bookmarks,customBmCats,objs,onSelect,onCheck,onUncheck,onAddBookmark,onRemoveBookmark,confetti}){
  ...
  const handleSvgClick=useCallback((e)=>{
    ...
    setPendingPos({mx:Math.round(mx*10)/10,my:Math.round(my*10)/10});
  },[bookmarkMode]);
    ...
    <div style={{position:"relative",background:T.canvasSoft,borderRadius:T.rLg,
      ...
      <div style={{padding:"10px 14px",borderBottom:`1px solid ${T.hairlineSoft}`,
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontFamily:FONT_MONO,fontSize:10,padding:"2px 7px",borderRadius:T.rFull,
            background:verified===total&&total>0?"rgba(55,205,132,0.1)":"rgba(255,255,255,0.06)",
            color:verified===total&&total>0?T.success:T.mute,
            border:`1px solid ${verified===total&&total>0?T.success:T.hairlineSoft}`}}>
        ...
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          ...
          <Eyebrow>Hover · click ✓ to verify</Eyebrow>
        </div>
      </div>

      {/* Bookmark mode banner */}
      {bookmarkMode&&<div style={{background:"rgba(243,100,88,0.07)",
        borderBottom:`1px solid rgba(243,100,88,0.25)`,
        padding:"7px 14px",display:"flex",alignItems:"center",gap:8}}>
        <span style={{width:6,height:6,borderRadius:"50%",background:T.brand,flexShrink:0,
          animation:"pulse 1s infinite"}}/>
        <span style={{fontFamily:FONT_MONO,fontSize:11,color:T.brand}}>
          Bookmark mode — click anywhere on the map to place a marker
        </span>
        <button onClick={()=>setBookmarkMode(false)} style={{marginLeft:"auto",background:"none",
          border:"none",cursor:"pointer",color:T.brand,display:"flex"}}><X size={13}/></button>
      </div>}

      {/* Progress bar */}
      <div style={{height:3,background:T.graphite}}>
        <div style={{height:"100%",background:T.success,
          width:`${total>0?(verified/total)*100:0}%`,transition:"width 0.6s cubic-bezier(0.4,0,0.2,1)"}}/>
      </div>

      {/* SVG Map */}
      <svg ref={svgRef} viewBox="0 0 100 100"
        style={{width:"100%",height:"auto",display:"block",cursor:bookmarkMode?"crosshair":"default"}}
        ...
        {/* Streets */}
        <rect x="0" y="0" width="100" height="8" fill="#181818" stroke="#252525" strokeWidth="0.3"/>
        <text x="50" y="5.5" textAnchor="middle" fontSize="2.6" fill="#4a4a4a" fontFamily="IBM Plex Mono,monospace">JARVIS CIRCUIT</text>
        <rect x="92" y="0" width="8" height="100" fill="#181818" stroke="#252525" strokeWidth="0.3"/>
        <text x="96" y="50" textAnchor="middle" fontSize="2.6" fill="#4a4a4a" fontFamily="IBM Plex Mono,monospace" transform="rotate(90,96,50)">HALIFAX STREET</text>

        {/* Park boundary */}
        <rect x="5" y="12" width="84" height="82" rx="1.5" fill="#121a0e" stroke="#1e2e18" strokeWidth="0.5"/>

        {/* Contour / ground tones */}
        <ellipse cx="45" cy="70" rx="35" ry="20" fill="#0f1a0c" opacity="0.5"/>
        <ellipse cx="20" cy="82" rx="14" ry="9" fill="#0a1208" opacity="0.7"/>

        {/* Tree canopy clusters */}
        {[
          [15,22,5.5],[10,50,4.5],[12,38,3.5],[30,28,4],[48,72,5],[60,82,4.5],
          [18,80,3.5],[52,85,3],[22,68,3],[75,60,3.5]
        ].map(([cx,cy,r],i)=>(
          <g key={`tree-${i}`} opacity="0.85">
            <circle cx={cx} cy={cy} r={r} fill="#16220f" stroke="#1e2e18" strokeWidth="0.4"/>
            <circle cx={cx-r*0.25} cy={cy-r*0.25} r={r*0.55} fill="#1a2812" opacity="0.6"/>
            <circle cx={cx+r*0.2} cy={cy-r*0.15} r={r*0.35} fill="#1e3015" opacity="0.4"/>
          </g>
        ))}

        {/* Lawn zone */}
        <rect x="8" y="28" width="26" height="28" rx="2" fill="#152010" opacity="0.5"/>
        <text x="21" y="44" textAnchor="middle" fontSize="2" fill="#2a4020" fontFamily="IBM Plex Mono,monospace" fontStyle="italic">open lawn</text>

        {/* Paths */}
        <path d="M 15 12 Q 28 36, 46 46 Q 64 56, 80 94" fill="none" stroke="#252525" strokeWidth="1.5" strokeDasharray="2,1.2"/>
        <path d="M 5 50 L 89 50" fill="none" stroke="#202020" strokeWidth="0.9"/>
        <path d="M 46 12 L 46 94" fill="none" stroke="#1e1e1e" strokeWidth="0.6" strokeDasharray="1.5,1" opacity="0.6"/>
        <path d="M 5 72 L 89 72" fill="none" stroke="#1e1e1e" strokeWidth="0.5" strokeDasharray="1.5,1" opacity="0.5"/>

        {/* Desire line F1 */}
        <path d="M 52 30 L 88 50" fill="none" stroke="#f36458" strokeWidth="0.8" strokeDasharray="1.5,1.2" opacity="0.5"/>
        <text x="70" y="37" textAnchor="middle" fontSize="1.6" fill="#f36458" fontFamily="IBM Plex Mono,monospace" opacity="0.5" transform="rotate(-20,70,37)">desire line</text>

        {/* Drainage low corner */}
        <ellipse cx="20" cy="82" rx="13" ry="8" fill="#091508" stroke="#122010" strokeWidth="0.5" opacity="0.8"/>
        <text x="20" y="85" textAnchor="middle" fontSize="1.7" fill="#2a4028" fontFamily="IBM Plex Mono,monospace" fontStyle="italic">drainage</text>

        {/* Playground */}
        <rect x="38" y="13" width="28" height="19" rx="2" fill="#1a1a14" stroke="#2a2a22" strokeWidth="0.4"/>
        <text x="52" y="19" textAnchor="middle" fontSize="2" fill="#555" fontFamily="IBM Plex Mono,monospace">Playground</text>
        {[[42,22,0.9,"#3a3a2a"],[46,21,0.7,"#444"],[50,22.5,0.9,"#3a3a2a"],[54,21,0.7,"#444"],[58,22,0.8,"#3a3a2a"]].map(([px,py,r,fill],i)=>(
          <circle key={i} cx={px} cy={py} r={r} fill={fill} opacity="0.7"/>
        ))}
        {/* Slide */}
        <line x1="40" y1="27" x2="43" y2="24" stroke="#444" strokeWidth="0.8"/>

        {/* Skate area */}
        <rect x="26" y="53" width="22" height="19" rx="2" fill="#161610" stroke="#222" strokeWidth="0.4"/>
        <ellipse cx="37" cy="63" rx="8" ry="5" fill="#1a1a14" stroke="#252520" strokeWidth="0.5" opacity="0.8"/>
        <text x="37" y="74" textAnchor="middle" fontSize="2" fill="#555" fontFamily="IBM Plex Mono,monospace">Skate area</text>
        <path d="M 31 60 Q 37 56, 43 60 Q 37 64, 31 60" fill="none" stroke="#2a2a22" strokeWidth="0.5" opacity="0.6"/>

        {/* BBQ / seating node */}
        <rect x="57" y="59" width="17" height="13" rx="1.5" fill="#1a1a14" stroke="#252520" strokeWidth="0.4"/>
        <rect x="59" y="61" width="4.5" height="3" rx="0.5" fill="#2a2a20" stroke="#333" strokeWidth="0.3"/>
        <rect x="65" y="61" width="4.5" height="3" rx="0.5" fill="#2a2a20" stroke="#333" strokeWidth="0.3"/>
        <circle cx="61.5" cy="63" r="1.2" fill="#3a3028" opacity="0.7"/>
        <circle cx="67.5" cy="63" r="1.2" fill="#3a3028" opacity="0.7"/>
        <text x="65" y="73.5" textAnchor="middle" fontSize="1.8" fill="#555" fontFamily="IBM Plex Mono,monospace">BBQ / seating</text>

        {/* Towers */}
        <rect x="70" y="13" width="18" height="15" rx="1" fill="#222222" stroke="#383838" strokeWidth="0.4"/>
        <text x="79" y="21.5" textAnchor="middle" fontSize="2" fill="#777" fontFamily="IBM Plex Mono,monospace">Tower 2</text>
        {[0,1,2].map(row=>[0,1,2,3].map(col=>(
          <rect key={`t2-${row}-${col}`} x={71.5+col*3.8} y={14.5+row*3} width="2.8" height="2.2"
            rx="0.3" fill={((row+col)%3===0)?"#3a3a28":"#1a1a1a"} opacity="0.8"/>
        )))}
        <rect x="70" y="32" width="18" height="15" rx="1" fill="#222222" stroke="#383838" strokeWidth="0.4"/>
        <text x="79" y="40.5" textAnchor="middle" fontSize="2" fill="#777" fontFamily="IBM Plex Mono,monospace">Tower 3</text>
        {[0,1,2].map(row=>[0,1,2,3].map(col=>(
          <rect key={`t3-${row}-${col}`} x={71.5+col*3.8} y={33.5+row*3} width="2.8" height="2.2"
            rx="0.3" fill={((row*2+col)%3===0)?"#3a3a28":"#1a1a1a"} opacity="0.8"/>
        )))}

        {/* Shadow cast line from towers */}
        <path d="M 70 27 L 60 45" fill="none" stroke="#0a0a0a" strokeWidth="4" opacity="0.5"/>
        <path d="M 88 27 L 78 45" fill="none" stroke="#0a0a0a" strokeWidth="3" opacity="0.4"/>

        {/* Compass */}
        <g transform="translate(87,90)">
          <circle cx="0" cy="0" r="4.5" fill="#181818" stroke="#2a2a2a" strokeWidth="0.5"/>
          <polygon points="0,-3.5 0.8,0 0,3.5 -0.8,0" fill="#444" opacity="0.6"/>
          <polygon points="0,-3.5 0.8,0 0,0 -0.8,0" fill="#f36458" opacity="0.85"/>
          <text x="0" y="-5" textAnchor="middle" fontSize="1.8" fill="#888" fontFamily="IBM Plex Mono,monospace">N</text>
        </g>

        {/* Scale bar */}
        <g transform="translate(8,97)">
          <line x1="0" y1="0" x2="16" y2="0" stroke="#444" strokeWidth="0.6"/>
          <line x1="0" y1="-0.8" x2="0" y2="0.8" stroke="#444" strokeWidth="0.6"/>
          <line x1="16" y1="-0.8" x2="16" y2="0.8" stroke="#444" strokeWidth="0.6"/>
          <text x="8" y="-1.5" textAnchor="middle" fontSize="1.8" fill="#555" fontFamily="IBM Plex Mono,monospace">~50m</text>
        </g>

        {/* Bookmark markers (triangles) */}
        {bookmarks.map(b=>{
          ...
          return(<g key={b.id} style={{cursor:"pointer"}}
            ...
            <circle cx={b.mx} cy={b.my} r="3.5" fill={col} opacity="0.1" stroke="none">
              <animate attributeName="r" values="3.5;5;3.5" dur="3s" repeatCount="indefinite"/>
            </circle>
            <polygon points={`${b.mx},${b.my-3.5} ${b.mx+2.5},${b.my+1.5} ${b.mx-2.5},${b.my+1.5}`}
              fill={col} opacity={isH?1:0.8} stroke={T.canvas} strokeWidth="0.5"/>
            <text x={b.mx} y={b.my-5} textAnchor="middle" fontSize="1.9"
              fill={col} fontFamily="IBM Plex Mono,monospace" opacity="0.9">{b.cat[0]}</text>
          </g>);
        })}
        ...
        {findings.map(f=>{
          ...
          return(<g key={f.id} style={{cursor:"pointer"}}
            ...
            onClick={e=>{if(!isChecked&&!bookmarkMode)onSelect(f);}}>
            {/* Pulse ring */}
            ...
            {/* Flash ring */}
            {isFlash&&<circle cx={f.mx} cy={f.my} r="4" fill="none"
              stroke={T.success} strokeWidth="1.5" opacity="0.9">
              <animate attributeName="r" values="4;10;14" dur="0.9s" fill="freeze"/>
              <animate attributeName="opacity" values="0.9;0.4;0" dur="0.9s" fill="freeze"/>
            </circle>}
            {/* Main dot */}
            <circle cx={f.mx} cy={f.my} r="2.3"
              fill={isFlash?"#37cd84":col}
              stroke={isChecked?T.success:T.canvas} strokeWidth="0.5"
              opacity={isChecked?0.55:1}/>
            {/* Check mark */}
            {isChecked&&<text x={f.mx} y={f.my+1} textAnchor="middle" fontSize="3"
              fill={T.success} fontFamily="sans-serif" opacity="0.8">✓</text>}
            {/* Label */}
            <text x={f.mx} y={f.my-4.2} textAnchor="middle" fontSize="2.2"
              fill={isChecked?T.success:T.col[f.status].text}
              fontFamily="IBM Plex Mono,monospace" fontWeight="500"
              opacity={isChecked?0.6:1}>{f.id}</text>
            {/* Check button on hover (only if not checked) */}
            {!isChecked&&isHov&&!bookmarkMode&&(
              <g onClick={e=>handleCheck(f,e)} style={{cursor:"pointer"}}>
                <circle cx={f.mx+5.5} cy={f.my-4.5} r="3" fill={T.success}
                  stroke={T.canvas} strokeWidth="0.5" opacity="0.95"/>
                <text x={f.mx+5.5} y={f.my-3.5} textAnchor="middle" fontSize="3.2"
                  fill={T.canvas} fontFamily="sans-serif" fontWeight="bold">✓</text>
              </g>
            )}
            {/* Unresolve button on hover (only if checked) */}
            {isChecked&&isHov&&!bookmarkMode&&(
              <g onClick={e=>{e.stopPropagation();onUncheck(f.id);}} style={{cursor:"pointer"}}>
                <circle cx={f.mx+5.5} cy={f.my-4.5} r="3" fill={T.graphite}
                  stroke={T.ash} strokeWidth="0.5" opacity="0.95"/>
                <text x={f.mx+5.5} y={f.my-3.4} textAnchor="middle" fontSize="3.2"
                  fill={T.ash} fontFamily="sans-serif">↩</text>
              </g>
            )}
          </g>);
        })}
      ...
      {/* Finding tooltip — position dynamically based on marker location */}
      {hf&&!bookmarkMode&&(()=>{
        // Flip vertically: markers in top 45% of map → tooltip below; otherwise above
        const tipTop = hf.my < 45;
        // Flip horizontally: markers in right 35% → tooltip on left side
        const tipLeft = hf.mx > 65;
        const headerH = 80; // approx px offset for header+banner+progressbar
        const svgEl = svgRef.current;
        const svgH = svgEl ? svgEl.getBoundingClientRect().height : 400;
        const markerPct = hf.my / 100;
        const markerPx = headerH + markerPct * svgH;
        return(
          <div style={{
            position:"absolute",
            top: tipTop ? `${markerPx + 18}px` : "auto",
            bottom: tipTop ? "auto" : `${svgH - markerPx + 18}px`,
            right: tipLeft ? "auto" : 12,
            left: tipLeft ? 12 : "auto",
            width:272,
            background:T.canvasSoft,
            border:`1px solid ${hf.checked?T.success:T.hairlineSoft}`,
            borderRadius:T.rMkt,padding:14,
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,flexWrap:"wrap"}}>
              ...
              {hf.checked&&<span style={{fontFamily:FONT_MONO,fontSize:9,color:T.success,
                padding:"1px 6px",borderRadius:T.rFull,border:`1px solid ${T.success}`}}>✓ Verified</span>}
            </div>
            <p style={{fontFamily:FONT_SANS,fontSize:12,fontWeight:500,color:T.onPrimary,
              margin:"0 0 4px",lineHeight:1.4}}>{hf.q}</p>
            <p style={{fontFamily:FONT_SANS,fontSize:11,color:T.ash,margin:0,lineHeight:1.4}}>{hf.ev}</p>
            {!hf.checked&&<Eyebrow style={{color:T.brand,display:"block",marginTop:6}}>
              Hover ✓ to verify · click dot to edit
            </Eyebrow>}
          </div>
        );
      })()}

      {/* Bookmark tooltip — also dynamic */}
      {hb&&(()=>{
        const tipTop = hb.my < 45;
        const tipLeft = hb.mx > 65;
        const headerH = 80;
        const svgEl = svgRef.current;
        const svgH = svgEl ? svgEl.getBoundingClientRect().height : 400;
        const markerPx = headerH + (hb.my/100)*svgH;
        return(
          <div style={{
            position:"absolute",
            top: tipTop ? `${markerPx+18}px` : "auto",
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <span style={{width:8,height:8,borderRadius:1,
                  background:T.bm[hb.cat]?.color||T.ash,transform:"rotate(45deg)"}}/>
              ...
              <button onClick={()=>onRemoveBookmark(hb.id)} style={{background:"none",border:"none",
                cursor:"pointer",color:T.mute,display:"flex",padding:2,transition:"color 0.15s",
                pointerEvents:"all"}}
      ...
      <div style={{padding:"8px 14px",borderTop:`1px solid ${T.hairlineSoft}`,
        ...
        <Eyebrow style={{color:T.mute}}>Findings</Eyebrow>
        {STATUSES.map(s=><div key={s} style={{display:"flex",alignItems:"center",gap:4}}>
          <span style={{width:6,height:6,borderRadius:"50%",background:T.col[s].border}}/>
          <span style={{fontFamily:FONT_MONO,fontSize:9,color:T.mute}}>{T.col[s].label}</span>
        </div>)}
        <span style={{width:1,height:10,background:T.hairlineSoft,flexShrink:0}}/>
        <Eyebrow style={{color:T.mute}}>Bookmarks</Eyebrow>
        {allBmCats.map(c=><div key={c} style={{display:"flex",alignItems:"center",gap:4}}>
...
function FCard({f,objs,onStatus,onEdit,onDragStart,checkFlash,onUncheck}){
  ...
  return(<div draggable onDragStart={e=>onDragStart(e,f.id)}
    style={{
      background:isFlash?"rgba(55,205,132,0.08)":f.checked?"rgba(55,205,132,0.03)":T.canvasSoft,
      ...
      boxShadow:isFlash?`0 0 24px rgba(55,205,132,0.25), 0 0 48px rgba(55,205,132,0.1)`:"none",
      opacity:f.checked?0.65:1}}
    onMouseEnter={e=>{if(!f.checked&&!isFlash){e.currentTarget.style.borderColor=T.hairline;e.currentTarget.style.boxShadow="0 4px 16px rgba(0,0,0,0.4)";}}}
    onMouseLeave={e=>{if(!isFlash){e.currentTarget.style.borderColor=f.checked?T.successDark:T.hairlineSoft;e.currentTarget.style.boxShadow="none";}}}>
    ...
    <p style={{fontFamily:FONT_SANS,fontSize:13,fontWeight:400,
      ...
      textDecoration:f.checked?"line-through":"none",letterSpacing:"-0.1px"}}>{f.q}</p>
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:6,marginTop:4}}>
      ...
      <div style={{display:"flex",gap:4,alignItems:"center"}}>
        {f.checked&&<button onClick={()=>onUncheck(f.id)} title="Unresolve — mark as not yet verified"
          style={{background:"none",border:`1px solid ${T.hairlineSoft}`,
            borderRadius:T.rSm,padding:"3px 7px",cursor:"pointer",display:"flex",
            alignItems:"center",gap:4,color:T.mute,fontFamily:FONT_MONO,fontSize:9}}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
      {[
        {title:`Unresolved (${unresolved.length})`,icon:<AlertTriangle size={14} style={{color:T.brand}}/>,col:T.brand,
         empty:"All findings resolved",emptyCol:T.success,
         items:unresolved.map(f=>({id:f.id,cat:f.cat,text:f.q,border:T.col[f.status].border}))},
        {title:`Uncovered (${uncovered.length})`,icon:<Eye size={14} style={{color:T.brand}}/>,col:T.brand,
         empty:"All objectives covered",emptyCol:T.success,
         items:uncovered.map(o=>({id:o.id,text:o.name,border:T.brand}))},
        {title:`Addressed (${designed.length})`,icon:<Check size={14} style={{color:T.col.designed.border}}/>,col:T.col.designed.border,
         empty:"No findings designed-for yet",emptyCol:T.mute,
         items:designed.map(f=>({id:f.id,cat:f.cat,text:f.dr||"No response recorded yet",border:T.col.designed.border}))},
      ].map((panel,i)=>(
        <div key={i} style={{background:T.canvasSoft,border:`1px solid ${T.hairlineSoft}`,borderRadius:T.rLg,padding:18}}>
          ...
          {panel.items.length===0&&<p style={{fontFamily:FONT_MONO,fontSize:11,
            color:panel.emptyCol,fontStyle:"italic"}}>{panel.empty}</p>}
        </div>
      ))}
    </div>
  </div>);
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,background:T.canvas}}>
      <div style={{background:T.canvas,borderBottom:`1px solid ${T.hairlineSoft}`,
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          <button onClick={()=>{setFindings(DEMO_F);setObjs(DEMO_OBJ);setBookmarks(DEMO_BM);setCustomBmCats([]);}}
      ...
      <div style={{flex:1,overflowY:"auto",padding:16}}>
        {view==="map"&&<SiteMap findings={filtered} bookmarks={bookmarks}
          customBmCats={customBmCats} objs={objs} onSelect={f=>setEditing(f)}
          onCheck={handleCheck} onUncheck={handleUncheck} onAddBookmark={handleAddBookmark}
